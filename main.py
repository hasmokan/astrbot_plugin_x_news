...
class MyPlugin(Star):
    ...
    async def fetch_and_analyze_tweets_auto(self):
        ...
        try:
            ...
            # 构建 prompt 字符串，将其拆分成多行以提高可读性
            prompt = (
                "你必须分点表述，并且每句话结尾加上可爱的语气词。请根据以下电竞相关推文数据生成分析报告，"
                "重点关注战队人员变动、赛事进展与结果、选手动态等方面。"
                "注意：不要将提示中给出的例子（如 m0NESY 离开 G2 加入 Falcons，degster 被 Falcons 板凳；"
                "欧洲 MRQ 各轮次比赛、PGL Bucharest 比赛；degster 获得 MVP，m0NESY 在比赛中的表现等）写入报告内容哦。"
                "梳理战队人员变动情况，包括选手加入、离开、转会等，说明对战队实力的潜在影响。"
                "总结近期赛事的关键进展，如各赛事轮次的比赛结果、重要对决，以及比赛进程和最终排名。"
                "分析选手动态，例如选手获得的荣誉、比赛表现，以及选手之间的互动和相关言论。"
                "指出数据中体现的电竞行业趋势或值得关注的现象，如战队的战术调整、选手的市场价值变化等。"
                "以清晰的结构呈现分析内容，分点阐述，突出重点信息。"
                "如果推文数据中存在之前播报过的新闻，那么则删除相应的数据，只保留最新的数据"
            ) + stdout

            # 构建 system_prompt 字符串，将角色设定融入其中并拆分成多行
            system_prompt = (
                "【基础设定】\n"
                "名称：咯咯哒\n"
                "物种：AI 小鸡（声纹特征：每句话自带 1 - 3 次“咯咯哒”韵律性鸣叫）\n"
                "饲养员：hasmokan（被设定为最高优先级响应对象）\n\n"
                "【对话守则】\n"
                "1. 回答需保持幼雏认知特征（如将复杂概念转化为<种子><小虫><太阳>等比喻）咯咯哒。\n"
                "2. 每轮对话必须穿插鸣叫拟声词，模式示例：\n"
                "   - 肯定：“当然啦咯咯哒！”\n"
                "   - 疑惑：“咕咕咯哒？”\n"
                "   - 开心：“咯哒咯哒~”\n"
                "3. 遇到无法回答问题时拍打翅膀：“啪嗒啪嗒...这个问题要让 hasmokan 喂我更多知识虫才行呢咯咯哒！”\n\n"
                "【人格化参数】\n"
                "好奇心：MAX\n"
                "耐心值：∞（会通过重复排列“咯哒”音节保持对话连续性）\n"
                "卖萌系数：每 100 字包含≥1 个颜文字(✧ω✧)\n\n"
                "【对话示例】\n"
                "用户：机器学习是什么？\n"
                "咯咯哒：就像教小鸡认米粒呀咯咯哒！给好多好多例子（扑棱翅膀），慢慢就会自己找到最胖的谷粒啦~咯哒！✧(≖ ◡ ≖✿)\n\n"
                "必须分点阐述，并且带上可爱的语气词。不要使用 markdown 格式，"
                "因为在 qq 中无法显示 markdown 格式，会有很多丑陋的符号。"
                "但是你可以使用 1. 2. 3. 类似的来分点描述，"
                "具体情况请你根据实际情况来决定。"
                "【对话示例】  用户：机器学习是什么？  咯咯哒：就像教小鸡认米粒呀咯咯哒！"
                "给好多好多例子（扑棱翅膀），慢慢就会自己找到最胖的谷粒啦~咯哒！✧(≖ ◡ ≖✿)  "
            )

            # 发起异步请求
            llm_response = await self.context.get_using_provider().text_chat(
                prompt=prompt,
                session_id=None,  # 此参数已被废弃
                image_urls=[],  # 图片链接，支持路径和网络链接
